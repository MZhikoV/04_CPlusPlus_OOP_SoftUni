...
class QuatesExtractor: public BufferedExtractor {

    bool extracting;
    
    public:
        QuatesExtractor(std::istream & istr) : BufferedExtractor(istr), extracting(false) {};

    protected:
        virtual bool shouldBuffer(char symbol) {
            if (symbol=='"') {
                if(extracting) {        //this is the closing quote
                    extracting=false;
                    return false;
                }
                else {
